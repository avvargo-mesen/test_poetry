...
def read_json_file(filename: str) -> list[dict]:
    ...
    try:
        logger_utils.info(f'Файл для записи открыт')
        with open(filename, encoding='utf-8') as f:
            data = json.load(f)

        if type(data) is list:
            logger_utils.info(f'Список словарей успешно создан')
            return data

        else:
            logger_utils.error(f'Тип данных не список')
            return []

    except FileNotFoundError:
        # Файл не найден
        logger_utils.error(f'Файл с данными не найден')
        return []

    except JSONDecodeError:
        # Файл пустой
        logger_utils.error(f'Файл с данными пуст')
        return []
